...
def create_events_source_kafka(t_env: StreamTableEnvironment) -> str:
    ...
    source_ddl = f"""
        CREATE TABLE {table_name} (
            ...
            'format' = 'json',
            'scan.watermark.idle-timeout' = '10 s'
        );
        """
...
def create_events_aggregated_sink(t_env: StreamTableEnvironment) -> str:
    ...
    Returns:
        The registered table name ('processed_events_green_flink').
    """

    table_name = 'processed_events_green_flink'
    sink_ddl = f"""
        CREATE TABLE {table_name} (
...
def log_aggregation():
    ...
    and runs a 1-hour tumbling window aggregation that counts trips and
    sums revenue per hour, writing results to PostgreSQL.
    """
